...
namespace DVS.Common.SO
{
    ...
    public class BasicSOInit
    {
        ...
        public static bool InitSO()
        {
            ...
            try
            {
                ...
                string randomStr = Utils.GetRandomString(8, false, true);
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception(msg);
                //logger.LogError(string.Format("SO初始化失败,{0}", ex.Message));
                //Process.GetCurrentProcess().Kill();
            }
    ...
    public class BasicSO
    {
        ...
        static BasicSO()
        {
            // ILogger<BasicSOService> logger, IConfiguration configuration
            //this.logger = logger;
            //this.configuration = configuration;
            //open = this.configuration.GetValue<string>("OpenSO");
        ...
        /// 加密
        /// </summary>
        /// <param name="plaiText"></param>
        /// <returns></returns>
        public static string Encrypt(string plain)
        {

            if (configuration["OpenSO"] != "0" && !File.Exists("/usr/lib64/libcloak.so"))
            {
                throw new ValidException("加密服务未安装");
            }
