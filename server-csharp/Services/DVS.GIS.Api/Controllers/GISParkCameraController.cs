...
namespace DVS.GIS.Api.Controllers.Console
{
    ...
    /// 园区摄像头信息
    /// </summary>
    [Route("/api/gis/console/park/camera")]
    public class GISParkCameraController : DvsControllerBase
    {
        ...
        public GISParkCameraController(IMapper mapper, IGISCameraService service)
        {
            this.mapper = mapper;
        ...
        /// <returns></returns>
        [HttpPost("list")]
        [PermissionFilter("企业摄像头标记管理", 128)]
        ...
        /// <returns></returns>
        [HttpPost("import")]
        [PermissionFilter(ModuleCodes.企业摄像头标记管理, PermissionCodes.Import)]
        public async Task<GISImportRes> ImportAsync(IFormFile excelFile, int areaId, int parkId)
        {
            ...
            return await this.cameraService.ImportExcelAsync(fileStream, category, loginuser.UserId, areaId, parkId);
        }
        ...
        /// <returns></returns>
        [HttpGet("export")]
        [PermissionFilter(ModuleCodes.企业摄像头标记管理, PermissionCodes.Export)]
        public async Task<FileContentResult> ExportAsync(int areaId, string ids = "", int typeId = 0, string keyword = "", int parkId = 0)
        {
            ...
            byte[] exportBytes = await this.cameraService.GetExcelData(areaId, typeId, keyword, ids, category, parkId);
